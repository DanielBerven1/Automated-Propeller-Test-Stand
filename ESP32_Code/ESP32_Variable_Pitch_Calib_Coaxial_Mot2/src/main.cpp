...
// From dc_motors.cpp
double ku_2 = 290.0; double tu_2 = 0.3;
double kp_2=354.5,ki_2=0.0,kd_2=0.1*ku_2*tu_2;
...
void callback(char* topic, byte* message, unsigned int length) {
  ...
  if (String(topic) == "rpi/DCSetpoint") {
    ...
    setSetpoint2(setpoint);
    current_error=getError2();
    c_position = getPosition2();
    D_print("SETPOINT:"); D_print(getSetpoint2()); D_print("   POS:"); D_print(c_position); D_print("    ERR:"); D_print(current_error); D_println("");
...
void loop() {
  ...
  if (pitch_angle_1 >= 0 and pitch_angle_2 >= 0){
    if (pitch_angle_1 >= 35.0 or pitch_angle_2 >= 35.0){
      setSetpoint2(0);
      setPIDgains2(0, 0, 0);
    }
  ...
  D_print("SETPOINT:"); D_print(getSetpoint2()); D_print("   POS:"); D_print(current_position); D_print("    ERR:"); D_print(current_error); 
