...
ESC=20  #Connect the ESC in this GPIO pin 
pi = pigpio.pi()
    ...
    if (first_time == False):
      ...
      while True:
         ...
         if new_pitch == True:
            ...
            Set_DC_Back_Setpoint(DC_setpoint)
...
def format_save_csv(pitch_setpoints):
   ...
   df.to_csv(path_Data + f'/{prop_type}_{prop_diameter}_raw_Stand2.csv', encoding = 'utf-8')

   MLP_calibrated_fits = pd.read_csv(path_Calib + f'/Stand2_MLP_calibrated_fits.csv')
   Torque_calibrated_fits = pd.read_csv(path_Calib + f'/Stand2_Torque_Force_Calibrated_Fits.csv')
   Thrust_calibrated_fits = pd.read_csv(path_Calib + f'/Stand2_Thrust_Calibrated_Fits.csv')
   ...
   df2.to_csv(path_Data + f'/{prop_type}_{prop_diameter}_VP_Stand2.csv', sep = ',', encoding = 'utf-8')
...
try:
   ...
   if inp == 'n': 
      ...
      print(f"Current DC Position is:  {DC_Back_position}")
